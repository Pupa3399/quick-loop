...
RAY_TEMP_ROOT="${OURO_RAY_TEMP_ROOT:-$PROJECT_ROOT/.venv/runtime/ray-ouro}"
...
export WANDB_DIR="${WANDB_DIR:-$EXPERIMENT_DIR/results/wandb}"
...
mkdir -p \
  ...
  "$PROJECT_ROOT/main/checkpoints/search_grpo/$RUN_NAME"
...
uv run --no-sync python -m verl.trainer.main_ppo \
  ...
  trainer.default_local_dir="$PROJECT_ROOT/main/checkpoints/search_grpo/$RUN_NAME" \
  ...
  2>&1 | tee "$EXPERIMENT_DIR/results/$RUN_NAME.log"
