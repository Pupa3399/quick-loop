...
from ouro_search.agent.profiles import AgentPrompt, PromptProfile, get_prompt_profile  # noqa: E402
...
from ouro_search.inference.qwen_hidden_loop import (  # noqa: E402
    HiddenLoopState,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 Qwen hidden residual R1/R2 预实验")
...
def prefix_turns(
    case: dict[str, Any],
    *,
    prefix_whitespace_mode: str,
) -> list[TurnRecord]:
    if prefix_whitespace_mode not in {"legacy", "no_added_newlines"}:
        raise ValueError(f"不支持的 prefix_whitespace_mode: {prefix_whitespace_mode}")
    action_separator = "" if prefix_whitespace_mode == "no_added_newlines" else "\n"
    ...
    for turn_id, source in enumerate(case["prefix_history"]):
        ...
        turns.append(
            TurnRecord(
                ...
                model_output=(
                    f"<think>{thinking}</think>{action_separator}"
...
def build_experiment_prompt(
    ...
    *,
    ...
    if prefix_whitespace_mode == "legacy":
        return profile.build_prompt(question, turns)
    if prefix_whitespace_mode == "no_added_newlines":
        return AgentPrompt(
            messages=({"role": "user", "content": profile.build_user_prompt(question)},),
            continuation="".join(
                f"{turn.model_output}{turn.information}" for turn in turns
            ),
...
) -> tuple[str, list[HiddenLoopTokenRecord], str]:
    token_ids: list[int] = []
...
def token_diagnostics(records: list[HiddenLoopTokenRecord]) -> list[dict[str, Any]]:
    return [
        {
            "token_index": index,
            ...
            "logits_max_abs_difference": record.logits_max_abs_difference,
            "reference_rms": record.reference_rms,
            "hidden_rms": record.hidden_rms,
        }
...
def run_branch(
    *,
    ...
    prefix_whitespace_mode = str(
        OmegaConf.select(config, "agent.prefix_whitespace_mode", default="legacy")
    )
    fixed_prefix = prefix_turns(
        case,
        prefix_whitespace_mode=prefix_whitespace_mode,
    )
    ...
    for generated_turn_index in range(int(config.agent.max_search_turns) + 1):
        ...
        audit = audit_generation("search_r1", output)
        turn = {
            "turn": turn_id,
            "loop_depth": depth,
            "model_output": output,
            ...
            "valid_action": audit.valid_action,
        ...
        information, information_tokens = append_observation(
            engine,
            state,
            profile.format_observation(response),
            max_tokens=min(
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    count = len(rows)
    target_turns = [row["turns"][int(row["target_turn"])] for row in rows]
    category_em: dict[str, list[int]] = defaultdict(list)
    for row in rows:
        category_em[str(row["candidate_type"])].append(int(row["em"]))
    return {
        "trajectory_count": count,
        "sample_count": len({row["sample_id"] for row in rows}),
        "target_format_valid_rate": sum(turn["format_valid"] for turn in target_turns) / count,
        "target_valid_action_rate": sum(turn["valid_action"] for turn in target_turns) / count,
        "target_search_action_rate": sum(turn["action"] == "search" for turn in target_turns)
        / count,
        "answer_rate": sum(row["termination_reason"] == "answer" for row in rows) / count,
        "em": sum(int(row["em"]) for row in rows) / count,
        "category_em": {
            category: sum(values) / len(values)
            for category, values in sorted(category_em.items())
        },
        ...
        "retriever_invocations": sum(int(row["retriever_invocations"]) for row in rows),
        "average_target_tokens": sum(
            int(turn["num_generated_tokens"]) for turn in target_turns
        )
        / count,
...
def main() -> None:
    ...
    if str(config.model.attention_backend) != "sdpa":
        raise ValueError("hidden residual reference backend 必须使用 SDPA")

    cases = load_cases(project_path(str(config.input_file)))
    health = validate_retriever(config.retriever)
    engine = QwenHiddenLoopEngine.from_pretrained(
